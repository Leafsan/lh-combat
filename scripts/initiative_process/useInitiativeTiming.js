export async function handleInitiativeTimingItems() {
  ...
  let message = "<h3>이니셔티브 타이밍 행동</h3>";
  ...
  for (let combatant of combatants) {
    ...
    if (items.length > 0) {
        ...
        new Dialog({
          ...
          buttons: {
            ...
            cancel: {
              ...
              label: "사용하지 않음",
              callback: () => {
                const noActionMessage = `<p>${actor.name}은(는) 이니셔티브 타이밍에 사용할 아이템이나 스킬을 사용하지 않습니다.</p>`;
                message = message.concat(noActionMessage);
                resolve();
              },
            },
      ...
      // 이니셔티브 타이밍에 사용할 아이템이나 스킬이 없는 경우 메시지 누적
      const noActionMessage = `<p>${actor.name}는 이니셔티브 프로세스에 사용할 아이템이나 스킬이 없습니다.</p>`;
      message = message.concat(noActionMessage);
    }
  }
